...
gulp.task('copy-to-node-red', ['copy-to-lib'], function () {
  if (node_red_root) {
    return gulp.src(['lib/*.*'])
    .pipe(gulp.dest(node_red_root + '/node_modules/node-red-contrib-amqp2/lib'));
  }
});

// unit tests, more a fast integration test because at the moment it uses an external amqp2 server
